...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        if "(" not in downloaded_file_name:
            ...
            await bot.send_message(event.chat_id, "Installed Plugin `{}` successfully.".format(os.path.basename(downloaded_file_name)))
        else:
            await bot.send_message(event.chat_id, "Plugin `{}` has been pre-installed and cannot be installed.".format(os.path.basename(downloaded_file_name)))